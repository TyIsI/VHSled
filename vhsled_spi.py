...
def writestripWithBrightness(pixels,spidev,brightness):
	for i in range(0,len(pixels),1):
		start = 0
		end = len(pixels[0])
		step = 1
		if (i % 2 == 1):
			...
			step = -1
		for j in range(start,end,step):
			spidev.write(chr((pixels[i][j]>>16) & brightness))
			spidev.write(chr((pixels[i][j]>>8) & brightness))
			spidev.write(chr(pixels[i][j] & brightness))
	spidev.flush()
